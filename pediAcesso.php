...
if($btnSim){  
  ...
  $cadastrar_inscricao = "INSERT INTO inscricaoevento(cod_evento, situacao, cod_usuario) VALUES (' $cod_evento', 'Fila de espera', '$idUsuario')";
  ...
  <div id="content">
    ...
    <form method="POST">
      <p style="text-align: center;">
        Olá Usuário <?php echo $_SESSION['nome_user'];  ?> !

        <br>

        Ao requerer uma mudança de acesso você passará para o próximo nível de acesso de tipo de usuário, lembrando que a prioridade de acessos segue a seguinte ordem: Participante => Auxiliar de Coordenação => Facilitador => Coordenador

        <br>

        Lembrando que você é do tipo <?php echo $_SESSION['tipo_user'];  ?> !

        <br>
      </p>
        <?php 
          if ($tipo_usuario == 'Coordenador' || $tipo_usuario == 'Administrador') {
            echo "<p class= 'text-danger'>Você não pode mudar seu nível de acesso, já está no nível máximo</p>";
          }else{
            echo " 
            <h3 style='text-align: center;'> Deseja mudar nível de acesso?</h3>
              <br>
              <div class='row'>
                <div class='col-lg-6'>
